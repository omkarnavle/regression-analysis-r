...
│   ├── full_model_output.png                 ← Full OLS model summary output
│   ├── refined_model_output.png              ← Refined OLS model summary output
│   ├── residuals_over_time.png               ← Residuals vs time (autocorrelation check)
│   ├── residual_lag_plot.png                 ← Residual lag scatter plot
│   ├── histogram_of_residuals.png            ← Histogram of OLS residuals
│   ├── durbin_watson_test.png                ← DW test output
│   ├── gls_model_output.png                  ← GLS AR(1) model summary
│   ├── cochrane_orcutt_output.png            ← Manual Cochrane-Orcutt output
│   ├── ols_vs_gls_comparison.png             ← OLS vs GLS coefficient comparison
│   └── refined_gls_model.png                 ← Final refined GLS model output
│
├── .gitignore                                ← Excludes CSV files and R output
└── README.md                                 ← This file
```
...
To investigate whether **hotel type** (City Hotel vs Resort Hotel) has a statistically significant effect on the **Average Daily Rate (ADR)** after controlling for booking lead time and length of stay.
...
Since `hotel` has p = 2 categories, exactly p − 1 = 1 dummy is created. City Hotel is the reference category.

```r
df$hotel_dummy <- ifelse(df$hotel == "Resort Hotel", 1, 0)
```

**Boxplot — ADR distributions by hotel type:**

![ADR by Hotel Type](images/adr_boxplot_by_hotel_type.png)

The boxplot confirms a distributional difference in ADR between the two hotel types, justifying the dummy variable.
...
|---|---|---|
| `Weekly_Sales` (Y) | Numerical | Total weekly revenue per store (USD) |
...
CPI and Unemployment are strong negative predictors. `Fuel_Price` is not significant. Standard errors may be understated due to autocorrelation — must be tested.
...
A positive linear relationship between e(t) and e(t−1) confirms consecutive residuals are correlated.

**Plot 3 — Histogram of Residuals:**

![Histogram of Residuals](images/histogram_of_residuals.png)

Roughly symmetric but right-skewed, with a long positive tail from high-sales holiday weeks.
...
|---|---|---|
| DW Statistic | 1.9329 | Below 2 — positive autocorrelation |
| p-value | 0.003313 | Reject H₀ at 1% level |
...
| Fuel_Price | −12,248.4 | −15,601.4 | Still **insignificant** |
| CPI | −1,585.8 | −1,577.4 | Very stable |
| Unemployment | −41,215.0 | −41,448.0 | Strongest predictor |
...
| 4 | GLS (nlme) | AR(1) Phi = 0.034; corrected SEs; Fuel_Price still insignificant |
| 5 | Cochrane-Orcutt | rho = 0.033 — confirms GLS result |
| 6 | Refined Model | Temperature and Unemployment are the robust predictors |

> **Key Takeaway:** Even mild autocorrelation distorts standard errors. The GLS Cochrane-Orcutt correction restores valid inference. CPI and Unemployment are the dominant drivers of weekly retail sales.
...
1. Download datasets from Kaggle (see `data/dataset_info.md`)
2. Place `hotel_bookings.csv` and `Walmart.csv` in the repo root
3. Open `.Rmd` files in RStudio → **Knit → Knit to Word**
...
| `lmtest` | Durbin-Watson test (`dwtest()`) |
| `nlme` | GLS with AR(1) correlation (`gls()`) |
